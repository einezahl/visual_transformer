...
class Projector(nn.Module):
    ...
    def forward(
        ...
        Args:
            feature_map (torch.Tensor): Output of the feature extractor, of
            shape (batch_size, n_channel, feature_width*feature_height)
            visual_token (torch.Tensor): Output of the transformer, of shape
            (batch_size, n_token, n_channel)

        Returns:
            torch.Tensor: Fusion of the transformers output and the feature
            map, of shape (batch_size, n_channel, feature_width*feature_height)
        """
