...
class Transformer(nn.Module):
    ...
    def forward(self, visual_token: torch.Tensor) -> torch.Tensor:
        """The transformer is a self attention module, modelling the relationship between the
        different visual token

        Args:
            visual_token (torch.Tensor): The visual tokens calculated by the tokenizer, has the
            shape (batch_size, n_token, n_channel)

        Returns:
            torch.Tensor: Visual tokens after applied self attention, has the shape
